...
\begin{align}
	...
	\mathcal{O}(k)&\equiv-\frac{k}{2}u_{tt}(x,\bar{t}),\qquad{t<\bar{t}<t+k}.
	\tag{Forward Difference for $u_t$}
\end{align}
\begin{align}
	u_x(x,t)&= \frac{u(x+h,t)-u(x-h,t)}{2h} + \mathcal{O}(h),\\
	\mathcal{O}(h^2)&\equiv-\frac{h^2}{6}u_{xxx}(\bar{x},t),\qquad{x-h<\bar{x}<x+h}.
	\tag{Centered Difference for $u_x$}
\end{align}
\begin{align}
	u_{xx}(x,t)&= \frac{u(x+h,t)-2u(x,t)+u(x-h,t)}{h^2} + \mathcal{O}(h),\\
	\mathcal{O}(h^2)&\equiv-\frac{h^2}{12}u_{xxxx}(\bar{x},t),\qquad{x-h<\bar{x}<x+h}.
	\tag{Centered Difference for $u_xx$}
\end{align}
\begin{align}
	...
	\tag{Centered Difference for $u_xt$}
\end{align}
...
\begin{example}
	For (9.1), the truncation error is $\mathcal{O}(k)\,(k\rightarrow0)$, provided $u_{tt}$ is bounded. For (9.2), the truncation error is $\mathcal{O}(h^2)\,(h\rightarrow0)$, provided $u_{xxx}$ is bounded. For (9.4), the truncation error is $\mathcal{O}(h^2+k^2)\,(h\rightarrow0)$, provided $u_{xxxt}$ and $u_{xttt}$ are bounded; we omit as understood the $(h,k\rightarrow0)$.
\end{example}
